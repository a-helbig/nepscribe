...
gen_script <- function(datapath_conv, datapath_local, suf_version, suf_version_short, dataformat, subformat, datalist, prio, english, set_missings, parallel, further_training, education, children){
  ...
  if(dataformat== "R" & subformat == "Harmonized Spell Format"){
    scripts <- c(
      ...
      "",
      "# 2. Step: Load Employment data set and join it to biograhpy in order to get access to working hours variable. This variable will be used in step 6 for spell prioritisation of episode types",
      "",
      "# 3. Step: Load Voctrain data set and join it to biograhpy in order to get access to type of vocational training variable. This variable will also be used in the spell prioritisation process",
      "",
      "# 4. Step: Expand the data to a monthly format by multiplying each row of each respondent with the spell length in month (dur variable). This expansion to a month format is needed since there is no wave-based information for episodes in the biography file but only one observation per episode with start and end date. To identify which episode represents the main activity at each interview date (and therefore wave) this reshape is needed. It serves as the basis for the spell prioritization process in the next step.",
      "",
      ...
      "",
      "# 6. Step: Reduce rows in data set to interview date months by loading cohort profile and generating a month variable for each interview date. Then we left join biography data by ID_t and month. This will lead to a data set with 1 row per wave of each respondent (what we call: 'person-year-data set')",
      "",
      ...
      "",
      "# To construct a person-year data set where each row corresponds to one wave for each individual, a spell prioritization process is useful for identifying the principal spell in cases where multiple spells occur simultaneously.",
      "",
  ...
  if(dataformat== "R" & subformat == "Original Subspell Format"){
    scripts <- c("# NEPScribe R base script to transform NEPS SUF data from a spell format into a person-year structured format.",
                 ...
                 "",
                 "bio <- bio |>",
                 "  dtplyr::lazy_dt() |> # because of large data frame use this function from dtplyr for fastest R solution outside the data.table package",
                 "  group_by(ID_t, splink)  |> ",
  ...
  if(dataformat == "STATA" & subformat == "Harmonized Spell Format") {
    scripts <- c(
      ...
      "",
      "********************************************************************************",
      "* This is a basic data preparation script, that generates a data set in a person-year-format by taking the harmonized spells from the biography data set as a baseline and merges infos from other data sets. Edits to the script that fit to you research project are possibly necessary and recommended.",
      "",
      "* Approach:",
      "",
      "* 1. Step: Load Biography data set as a base for the person-year-data set and generate start-, enddate and duration variables",
