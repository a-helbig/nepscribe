...
gen_script <- function(datapath_conv, datapath_local, suf_version, dataformat, subformat, datalist, prio, english, set_missings, parallel, further_training, education, children, work_exp, unemp_exp){
      ...
      if(dataformat== "R" & subformat == "Harmonized Spell Format"){
        ...
        scripts <- c(
          ...
          "",
          "",
          if(sc=="SC4")"# Please note that SC4 was surveyed twice in 2012 and 2013. For further details, see: https://www.neps-data.de/Portals/0/NEPS/Datenzentrum/Forschungsdaten/SC4/16-0-0/NEPS_SC4_StudyOverview_w1-16_de.pdf",
          if(sc=="SC4")"# Also note, that there was no survey wave 2017/2018.",
          if(sc=="SC4")"",
          "# Approach:",
          "",
          ...
          if(nchar(datapath_local)==0)"suf_version = \"17-0-0\" # Please adjust if necessary",
      ...
      if(dataformat== "R" & subformat == "Original Subspell Format"){
        ...
        scripts <- c("# NEPScribe R base script to transform NEPS SUF data from a spell format into a person-year format.",
                     ...
                     "list.of.packages <- c('dplyr','haven','lubridate','tidyr','dtplyr', 'remotes', 'stringr', 'janitor')",
                     "new.packages <- list.of.packages[!(list.of.packages %in% installed.packages()[,'Package'])] # check if required packages are in installed packages on this machine",
                     "if(length(new.packages)) install.packages (new.packages) # if new.packages contains sth and thus length is > 0, install those new packages",
                     "",
                     "# install nepstools if not installed (package needs to be installed seperately from github) ",
                     ...
                     "bio <- bio |>",
                     "  replace_values_with_na(vars = c(spell_vars))",
                     "",
                     "bio <- bio |>",
                     ...
                     if(sc %in% c("SC5"))"    start = coalesce(start, ((tg3607y - 1960) * 12) + tg3607m - 1),",
                     ...
                     if(sc != "SC5")"    end = coalesce(end, ((ts2512y - 1960) * 12) + ts2512m - 1)",
                     if(sc %in% c("SC5"))"    end = coalesce(end, ((ts2512y - 1960) * 12) + ts2512m - 1),",
                     if(sc %in% c("SC5"))"    end = coalesce(end, ((tg3608y - 1960) * 12) + tg3608m - 1)",
                     "  ) |>",
                     ...
                     if(sc != "SC5")"            ts2911y,ts2311y,ts2111y,ts2711y,ts1511y,ts1111y,ts1311y,ts2511y,ts2312y,ts2912y,ts2112y,ts2712y,ts1512y,ts1112y,ts1312y,ts2512y)) # Remove original date columns",
                     if(sc %in% c("SC5"))"            ts2911y,ts2311y,ts2111y,ts2711y,ts1511y,ts1111y,ts1311y,ts2511y,ts2312y,ts2912y,ts2112y,ts2712y,ts1512y,ts1112y,ts1312y,ts2512y, tg3608m, tg3608y, tg3607m, tg3607y)) # Remove original date columns",
                     "",
                     "# Assign varlabels to variables",
                     if(english)"attr(bio$start, 'label') <- 'Start date'",
                     if(english)"attr(bio$end, 'label') <- 'End date'",
                     ...
                     "bio <- bio |> ",
                     if(sc %in% c("SC6"))"  filter(ts2312c == 1 | ts2912c == 1 | ts2112c == 1 | ts2712c == 1 | ts1512c == 1 | ts1112c == 1 | ts1312c == 1 | ts2512c == 1)",
                     if(sc %in% c("SC5"))"  filter(ts2312c == 1 | ts2912c == 1 | ts2112c == 1 | ts2712c == 1 | ts1512c == 1 | ts1112c == 1 | ts1312c == 1 | ts2512c == 1 | tg36109 == 1)",
                     ...
                     "# Drop unnecessary variables",
                     "bio <- bio  |> ",
                     "  select(-starts_with('ts'), -prio, -spms, -starts_with('ts15201'), -spgen, -spext, -spstat, -disagint, -disagwave)",
                     "",
      ...
      if(dataformat == "STATA" & subformat == "Harmonized Spell Format") {
        ...
        scripts <- c(
          ...
          if(nchar(datapath_local)==0)"global suf \"17-0-0\" // Please specify, e.g. 17-0-0",
          "",
          "",
          "********************************************************************************",
          "* 1. Base: biography file",
      ...
      if(dataformat == "STATA" & subformat == "Original Subspell Format"){
        ...
        scripts <- c(
          ...
          if(nchar(datapath_local)==0)"global DATA = \"\" // Specify local datapath here",
          if(nchar(datapath_local)>0)paste0("global suf ", "\"",suf_version, "\"", " // Please adjust if necessary"),
          if(nchar(datapath_local)==0)"global suf \"\" // Please specify, e.g. 17-0-0",
          "",
          ...
          if(sc == "SC5")"global keep_vars ts1111m ts1111y ts1112m ts1112y ts1112c ts1311m ts1311y ts1312m ts1312y ts1312c ts1511m ts1511y ts1512m ts1512y ts1512c ts2111m ts2111y ts2112m ts2112y ts2112c  ts2311m ts2311y ts2312m ts2312y ts2312c ts2511m ts2511y ts2512m ts2512y ts2512c  ts2711m ts2711y ts2712m ts2712y  ts2712c ts2911m ts2911y ts2912m ts2912y ts2912c ts15201 ts23223 tg3608m tg3608y tg36109 tg3607m tg3607y",
          ...
          if(sc == "SC5") "replace sptype = 36 if sptype == .",
          if(sc == "SC5") "",
          "label var sptype \"Spelltyp\"",
          ...
          if(sc == "SC5")"replace start = ym(tg3607y, tg3607m) if start == . // Intern",
          "",
          ...
          if(sc == "SC5")"replace end = ym(tg3608y, tg3608m) if end == . // Intern",
          "",
          "* Format this var",
          "format start %tm",
          "format end %tm ",
          ...
          if(sc %in% c("SC6")) "foreach var of varlist ts2312c ts2912c ts2112c ts2712c ts1512c ts1112c ts1312c ts2512c {",
          if(sc %in% c("SC5")) "foreach var of varlist ts2312c ts2912c ts2112c ts2712c ts1512c ts1112c ts1312c ts2512c tg36109  {",
          ...
          if(sc %in% c("SC3","SC4"))"drop ts1111m ts1111y ts1112m ts1112y tf1112c ts1311m ts1311y ts1312m ts1312y ts1312c ts1511m ts1511y ts1512m ts1512y ts1512c ts2111m ts2111y ts2112m ts2112y ts2112c ts2311m ts2311y ts2312m ts2312y ts2312c ts2511m ts2511y ts2512m ts2512y ts2512c ts2711m ts2711y ts2712m ts2712y ts2712c ts2911m ts2911y ts2912m ts2912y ts2912c prio worktime spgen spext spstat disagint disagwave spms",
          if(sc %in% c("SC6"))"drop ts1111m ts1111y ts1112m ts1112y ts1112c ts1311m ts1311y ts1312m ts1312y ts1312c ts1511m ts1511y ts1512m ts1512y ts1512c ts2111m ts2111y ts2112m ts2112y ts2112c ts2311m ts2311y ts2312m ts2312y ts2312c ts2511m ts2511y ts2512m ts2512y ts2512c ts2711m ts2711y ts2712m ts2712y ts2712c ts2911m ts2911y ts2912m ts2912y ts2912c prio worktime spgen spext spstat disagint disagwave spms",
          if(sc %in% c("SC5"))"drop ts1111m ts1111y ts1112m ts1112y ts1112c ts1311m ts1311y ts1312m ts1312y ts1312c ts1511m ts1511y ts1512m ts1512y ts1512c ts2111m ts2111y ts2112m ts2112y ts2112c ts2311m ts2311y ts2312m ts2312y ts2312c ts2511m ts2511y ts2512m ts2512y ts2512c ts2711m ts2711y ts2712m ts2712y ts2712c ts2911m ts2911y ts2912m ts2912y ts2912c tg3607m tg3607y tg3608m tg3608y tg36109 prio worktime spgen spext spstat disagint disagwave spms ",
          "",
        ...
        if (english) {
          scripts <- c(
            ...
            "label language en",
            "",
            "label var start \"Date of episode start (in months since january 1960)\"",
            "label var end \"Date of episode end (months since january 1960)\"",
