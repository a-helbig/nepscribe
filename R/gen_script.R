...
gen_script <- function(datapath_conv, datapath_local, suf_version, dataformat, subformat, datalist, prio, english, set_missings, parallel, further_training, education, children, work_exp, unemp_exp){
      ...
      if(dataformat== "R" & subformat == "Harmonized Spell Format"){
        scripts <- c(
          ...
          "# Approach:",
          "",
          "# 1. Step: Load biography data set as a base for the person-year-data set and compile start and end date and generate a duration variable.",
          "",
          "# 2. Step: Load employment data set and merge it to biography to access the working hours variable. This variable will be used in step 5 for the prioritisation of parallel employment episodes.",
          "",
          "# 3. Step: Load the VocTrain data set and merge it to the biography data set to access the vocational training type variable. This variable will also be used in the spell prioritisation process.",
          "",
          ...
          "",
          "# 6. Step: Reduce rows in data set to interview date months by loading the cohort profile data set and generating a month variable for each interview date. Then merge this data set and the biography data by ID_t and month. This leads to a data set with one row per wave of each respondent (what we call: 'person-year-data set').",
          "",
          ...
          "",
          "# Read NEPS data set with nepstools::read_neps()",
          ...
          "",
          "# To construct a person-year data set where each row corresponds to one wave for each individual, a spell prioritisation process is useful for identifying the principal spell in cases where multiple spells occur simultaneously.",
          "",
      ...
      if(dataformat== "R" & subformat == "Original Subspell Format"){
        scripts <- c("# NEPScribe R base script to transform NEPS SUF data from a spell format into a person-year structured format.",
                     ...
                     if(sc=='SC6')"fill_vars <- c('ts23223_g1', 'vt_typ') # working hours of employment and type of vocational training needed for later prioritization",
                     if(sc %in% c("SC3", "SC4","SC5"))"fill_vars <- c('ts23223', 'vt_typ') # working hours of employment and type of vocational training needed for later prioritization",
                     "",
                     "",
      ...
      if(dataformat == "STATA" & subformat == "Harmonized Spell Format") {
        scripts <- c(
          ...
          "* Edits to the script that fit to you research project are possibly necessary and recommended.",
          "",
          "********************************************************************************",
          "* If you want access to within spell variation across waves, spell related longitudinal information should be merged via 'ID_t', 'wave' and 'splink' after step 6, when the person-year structure has been generated and after dropping harmonized episodes in the spell files. When you use the 'Additional variables' function in the NEPScribe app, this process is used. It ensures that time-variant spell information is not being lost. However, it requires a handling of various missing value codes due to filtering or data issues. Examples are provided when using the 'Additional variables' function.",
          "********************************************************************************",
          "",
          "* Approach:",
          "",
          "* 1. Step: Load Biography data set as a base for the person-year-data set and generate start date, end date and duration variables.",
          "",
          "* 2. Step: Load Employment data set and merge it to biography in order to get access to working hours variable. This variable will be used in step 6 for spell prioritisation of episode types.",
          "",
          "* 3. Step: Load VocTrain data set and merge it to biography in order to get access to type of vocational training variable. This variable will also be used in the spell prioritisation process.",
          "",
          "* 4. Step: Expand the data to a monthly format by multiplying each row of each respondent with the spell length in month (dur variable). This expansion to a month format is needed since there is no wave-based information for episodes in the biography file but only one observation per episode with start and end date. To identify which episode represents the main activity at each interview date (and therefore wave) this reshape is needed. It serves as the basis for the spell prioritisation process in the next step.",
          "",
          "* 5. Step: Spell prioritisation process. Here, we sort the months of each respondent by determined spell order, main/side episode (descending), working hours (descending), employment duration (descending) and splink. You may edit this according to your research question(s).",
          "",
          "* 6. Step: Reduce rows in data set to interview date months by loading cohort profile and generating a month variable for each interview date. Then we merge this data set to biography data by ID_t and month and keep only matches and using data. This will lead to a data set with 1 row per wave of each respondent (what we call: 'person-year-data set').",
          "",
          "* Additional data preparation steps may vary based on the settings configured in the Transform Data tab of the NEPScribe app.",
          "",
          "********************************************************************************",
      ...
      if(dataformat == "STATA" & subformat == "Original Subspell Format"){
        scripts <- c(
          ...
          if(sc == "SC6")"global fill_vars ts23223_g1 vt_typ // working hours of employment and type of vocational training needed for later prioritization",
          if(sc %in% c("SC3","SC4","SC5"))"global fill_vars ts23223 vt_typ // working hours of employment and type of vocational training needed for later prioritization",
          "",
          "*************************************************************************",
          "  * 1. Append spelldata files",
          "*************************************************************************",
