...
gen_script <- function(datapath_conv, datapath_local, suf_version, suf_version_short, dataformat, subformat, datalist, prio, english, set_missings, parallel, further_training, education, children){
  ...
  if(dataformat== "R" & subformat == "Harmonized Spell Format (Recommended)"){
    scripts <- c(
      ...
      "# If you want acess to within spell variation across waves, spell related longitudinal information should be joined via 'ID_t', 'wave' and 'splink' after step 6, when the person-year structure has been generated and after dropping harmonized episodes in the spellfiles.",
      ...
      "",
      "# 2. Step: Load Employment dataset and join it to biograhpy in order to get access to working hours variable. This variable will be used in step 6 for spell prioritisation of episode types",
      "",
      "# 3. Step: Load Voctrain dataset and join it to biograhpy in order to get access to type of vocational training variable. This variable will also be used in the spell prioritisation process",
      "",
      "# 4. Step: Expand the data to a monthly format by multiplying each row of each respondent with the spell length in month (dur variable). This serves as the basis for the spell prioritization process in the next step.",
      "",
      ...
      "",
      "# 6. Step: Reduce rows in dataset to interview date months by loading cohort profile and generating a month variable for each interview date. Then we left join biography data by ID_t and month. This will lead to a dataset with 1 row per wave of each respondent (what we call: 'person-year-dataset')",
      "",
      ...
      "",
      "# After expanding the data to a monthly structure, which results in a very large dataframe, the script occasionally uses the lazy_dt() function from the dtplyr package to achieve the fastest R solution outside the data.table package.",
      "",
      ...
      "",
      "# Select Variables, generate a month variable and filter for participation waves and non-NA month rows",
  ...
  if(dataformat == "STATA" & subformat == "Harmonized Spell Format (Recommended)") {
    scripts <- c(
      ...
      "* 1. Step: Load Biography dataset as a base for the person-year-dataset and generate start-, enddate and duration variables",
      "",
      "* 2. Step: Load Employment dataset and merge it to biograhpy in order to get access to working hours variable. This variable will be used in step 6 for spell prioritisation of episode types",
      "",
      "* 3. Step: Load Voctrain dataset and merge it to biograhpy in order to get access to type of vocational training variable. This variable will also be used in the spell prioritisation process",
      "",
      "* 4. Step: Expand the data to a monthly format by multiplying each row of each respondent with the spell length in month (dur variable). This serves as the basis for the spell prioritization process in the next step.",
      "",
      "* 5. Step: Spell prioritisation process. Here, we sort the months of each respondent by determined spell order, main/side episode (descending), working hours (descending), employment duration (descending) and splink. You may edit this according to your research question(s).",
      "",
      "* 6. Step: Reduce rows in dataset to interview date months by loading cohort profile and generating a month variable for each interview date. Then we merge this dataset to biography data by ID_t and month and keep only matches and using data. This will lead to a dataset with 1 row per wave of each respondent (what we call: 'person-year-dataset')",
      "",
      "* Additional data preparation steps may vary based on the settings configured in the Transform Data tab of the SUF-Explorer",
      "",
      "********************************************************************************",
      ...
      "********************************************************************************",
      "",
      "* We also require the type of vocational training for the prioritisation process of spells later in Step 5. Thus we merge voctrain data, recode type of voc. training (different variable in Wave 1: ts15201_v1), reduce to harmonized episodes (subspell==0) and join it to biography data.",
      "",
      ...
      "drop worktime prio prio_temp wave ts23223 vt_typ",
      "",
      "********************************************************************************",
      "* 6. Create person year format",
      "********************************************************************************",
  ...
  if(dataformat == "STATA" & subformat == "Original Subspell Format"){
    scripts <- c(
      ...
      "",
      "*** data prep of indicators that are necessary for spell prioritisation later ***",
      if(sc=="SC6")"* harmonize type of voctraining",
      if(sc=="SC6")"* first we need to align the codes of both variables because they differ unfortunately",
      if(sc=="SC6")"clonevar vt_typ_v1 = ts15201_v1",
      if(sc=="SC6")"recode vt_typ_v1 (2=3) (3=4) (4=5) (5=6) (6=9) (7=10) (8=15) (9=16) (10=11) (11=12) (12=13) (13=14) (14=17)",
      ...
      "* Set Missings on variables used for prioritisation later. It is recommended to use the function nepsmiss from the nepstools ado to recode all missing values to specific missing codes. You may install it with: net install nepstools, from(http://nocrypt.neps-data.de/stata). If you want to exclude specific missings from being recoded you might use statas mvdecode function instead.",
      "nepsmiss _all",
      "",
