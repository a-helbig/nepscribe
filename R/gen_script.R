...
gen_script <- function(datapath_conv, datapath_local, suf_version, suf_version_short, dataformat, subformat, datalist, prio, english, set_missings, parallel, further_training, education, children){
  ...
  if(dataformat== "R" & subformat == "Harmonized Spell Format (Recommended)"){
    scripts <- c(
      ...
      "",
      "# 4. Step: Expand the data to a monthly format by multiplying each row of each respondent with the spell length in month (dur variable). This serves as the basis for the prioritization process in the next step.",
      "",
      "# 5. Step: Priorisation process. Here, we sort the months of each respondent by determined spell order, main/side episode (descending), working hours (descending), employment duration (descending) and splink. You may edit this according to your research question(s).",
      "",
      "# 6. Step: Reduce rows in dataset to interview date months by loading cohort profile, generating month variable and then left joining biography data by ID_t and month. This will lead to a dataset with 1 row per wave of each respondent (what we call: 'person-year-dataset')",
      "",
      ...
      "",
      "# We need the working hours for the priorisation process of parallel employment spells later in Step 5. Thus we join employment data, recode working hours and reduce to harmonized episodes (subspell==0) and join it to biography data.",
      ...
      "rm(emp)",
      "",
      "# 3. Vocational Training Preparation for Priorisation -----------------------------------------",
      "",
      if(sc == "SC6")"# We also require the type of vocational training for the priorisation process of spells later in Step 4. Thus we join voctrain data, recode type of voc. training (different variable in Wave 1: ts15201_v1), reduce to harmonized episodes (subspell==0) and join it to biography data.",
      ...
      if(sc == "SC6")"})",
      if(sc %in% c("SC3","SC4","SC5"))"# We also require the type of vocational training for the priorisation process of spells later in Step 5. Thus we join voctrain data, reduce to harmonized episodes (subspell==0) and join it to biography data.",
      ...
      "rm(voctrain)",
      "",
      "# 4. Expand to monthly structure -------------------------------------------",
      "",
      "# The expand function will generate one row for each month in the duration variable ('dur')",
      ...
      "",
      "# We  may recode licences courses and ihk courses to value 31 to place it behind xmodule episodes in the priorisation order, because these types of episodes are typically rather side spells than main spells and thus considered of lesser importance in many research projects with regard to spell priorisation. You may change that.",
      "",
      "bio <- bio |>",
      "  mutate(prio = sptype,",
  ...
  if(dataformat == "STATA" & subformat == "Harmonized Spell Format (Recommended)") {
    scripts <- c(paste0("* Base Dofile to transform NEPS ", sc," SUF data into a person-year format"),
      ...
      "* 5. Step: Priorisation process. Here, we sort the months of each respondent by determined spell order, main/side episode (descending), working hours (descending), employment duration (descending) and splink. You may edit this according to your research question(s).",
      "",
      "* 6. Step: Reduce rows in dataset to interview date months by loading cohort profile, generating month variable and then merge this dataset to biography data by ID_t and month and keep only matches and using data. This will lead to a dataset with 1 row per wave of each respondent (what we call: 'person-year-dataset')",
      "",
      "* Additional data preparation steps may vary based on the settings configured in the Transform Data tab of the SUF-Explorer",
      "",
      "********************************************************************************",
      ...
      "********************************************************************************",
      "",
      "* We need the working hours for the priorisation process of parallel employment spells later in Step 5. Thus we merge employment data, recode working hours and reduce to harmonized episodes (subspell==0) and join it to biography data.",
      "",
      ...
      "********************************************************************************",
      "",
      "* We also require the type of vocational training for the priorisation process of spells later in Step 5. Thus we merge voctrain data, recode type of voc. training (different variable in Wave 1: ts15201_v1), reduce to harmonized episodes (subspell==0) and join it to biography data.",
      "",
      ...
      "",
      "********************************************************************************",
      "* 4. Expand dataframe to 'months-structure'",
      "********************************************************************************",
      "",
  ...
  if(dataformat == "STATA" & subformat == "Original Subspell Format"){
    scripts <- c("* Dofile for generating dataset in person year format. Harmonized spells from biography are being ignored - instead the original spell files are taken",
      "",
      "********************************************************************************",
      "  *** !!!ATTENTION!!! ",
