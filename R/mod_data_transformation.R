...
#       filenames <- reactive({
#         req(cohort_path())
#         files <- list.files(cohort_path(), pattern = "*.dta", full.names = TRUE)
...
#       varlist <- reactiveValues(data = list())
...
#         withProgress(message = 'Processing data, please wait...', value = 0, {
#           showModal(shiny::modalDialog(
#             title = "Note",
#             size = "s",
#             HTML("Please wait while data is being processed. The resulting dataset is generated according to your settings and is identical to the dataset you would receive by downloading and executing the script."),
#             easyClose = TRUE,
#             footer = shiny::modalButton("Close")
#           ))
#           gen_data(
