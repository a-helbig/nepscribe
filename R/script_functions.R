...
gen_qualification_prep_code_r <- function(english, SC, suf_version,suf_version_short) {
  lines_of_code <- c(
    "# Educational Qualification ----------------------------------------------------------",
    "",
    "#AH: In the Education dataset, information is provided on educational attainment and vocational qualifications classified according to ISCED and CASMIN. These qualifications are recorded in a month/year format. We therefore need to restructure the dataset before merging it with our person-year dataset.",
    "",
    "# Load cohort profile to get access to interviewdates",
    ...
    "         isced = tx28103) ",
    "  ",
    "# Reshape edu_data to wide format",
    "edu_data <- edu_data |> ",
    ...
    "  min_suffix <- 1",
    "}",
    "# Generate cross-sectional variables indicating the CASMIN level achieved in wave x. Loop over all waves to create empty indicators casmin_wave_x. Then loop over all CASMIN values and replace each cross-sectional CASMIN indicator with the original CASMIN value if the CASMIN date is less than or equal to the interview date and the interview date is not missing",
