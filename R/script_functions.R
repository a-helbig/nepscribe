 ...
 generate_strings <- function(data_list, suf_version, english) {
  ...
  for (i in seq_along(data_list)) {
    ...
    if(sum(!is.na(df$variable))>0){
      ...
      string11 <- ""
      string12 <- if(stringr::str_detect(df[1, "Dataset"],  spstat_vars_regex)) {"stopifnot(1==2) # We stop script execution here, in order to draw your attention to the following Notes. You may delete this line now."}
...
generate_strings_stata <- function(data_list, suf_version) {
  ...
  for (i in seq_along(data_list)) {
    ...
    if(sum(!is.na(df$variable))>0){
      ...
      string16 <- ""
      string16 <- if(stringr::str_detect(df[1, "Dataset"],  spstat_vars_regex)) {"assert 1 == 2 // We stop script execution here, in order to draw your attention to the following notes. You may delete this line now."}
